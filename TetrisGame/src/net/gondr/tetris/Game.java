...
public class Game {
	...
	public Game(Canvas canvas) {
		...
		double size = (width - 4) / 10; //한칸한칸의 사이즈. 4칸은 인터페이스칸 10칸은 게임칸
		...
		mainLoop = new AnimationTimer() { // javafx에서 일정시간 간격으로 계속해서 작업을 실행해야할 때 쓰이는 쓰레드
			@Override
			public void handle(long now) { //now는 나노초 단위로 들어옴. 나노초 10억분의 1초 1/1,000,000,000
				update( (now - before) / 1000000000d ); //현재시간-전 시간 = 간격을 초단위로 바꿔서 업데이트에 전달해줌
				before = now;
				render();
			}
		};
